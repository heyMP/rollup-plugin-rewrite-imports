...
module.exports = function rewriteImports(appendPath) {
    const patternImport = new RegExp(/import\s+?(?:(?:((?:[\w*\s{},]*))\s+from\s+?)|)(?:(?:["|'](.*?)["|']))[\s]*?(?:;|$|)/, 'mg')
        ...
        renderChunk(code, info, opts) {
            ...
            function replaceImport() {
                ...
                if (fs.existsSync(path.join(process.cwd(), rollupOutputDir(opts, appendPath), match[2]))) {
                    ...
                    // delete the entire match because it's going to be bundled in one file
                    replacement = String(match[0].replace(match[0], ""))
                    magicString.overwrite(start, end, replacement)
                }
            }
